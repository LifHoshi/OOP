...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    List<CompletableFuture<String>> templist = new ArrayList<CompletableFuture<String>>();
    ...
    try {
      ...
      while (sc.hasNext()) {
        ...
        CompletableFuture<String> bus = BusSg.findBusServicesBetween(srcId, searchString)
      ...
      CompletableFuture<?>[] arrayAll = new CompletableFuture<?>[templist.size()];
      CompletableFuture.allOf(templist.toArray(arrayAll)).join();

      for (int i = 0; i < arrayAll.length; i++) {
        arrayAll[i].thenAccept(System.out::println); 
      }
      sc.close();
