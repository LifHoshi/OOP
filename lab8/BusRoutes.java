...
class BusRoutes {
  ...
  public CompletableFuture<String> description() {
    final String temp = "Search for: " + this.stop + " <-> " + name + ":\n"
        + "From " +  this.stop + "\n";

    CompletableFuture<String> result = CompletableFuture.completedFuture(temp);

    for (BusService service : services.keySet()) {
      CompletableFuture<Set<BusStop>> stops = services.get(service);

      CompletableFuture<String> des = stops.thenApply(x -> describeService(service, x));

      result = result.thenCombine(des, (x1, x2) -> x1 + x2);
    }
    return result;
  }
